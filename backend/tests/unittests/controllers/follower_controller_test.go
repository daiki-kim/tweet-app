...
func TestGetFollowsSuccess(t *testing.T) {
	...
	// GetFollowers APIを準備
	r.GET("/api/v1/follower/follows/:follower_id", testFollowerController.GetFollows)

	// リクエスト作成
	req, _ := http.NewRequest(http.MethodGet, "/api/v1/follower/follows/1", nil)
	...
	// モックサービスを準備
	mockFollowerService.On("GetFollows", uint(1)).Return(followerResponse, nil)
